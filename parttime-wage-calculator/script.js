...
function calc() {
  ...
  for (const wkKey of Object.keys(weeks)) {
    ...
    const hasNext = Object.keys(weeks).includes(nextWeekKey)
      ? hasAnyPlannedWork(weeks[nextWeekKey])
      : false
    const sundayInside = wk.some(it => it.isSunday &&
...
function showResult(basePay, jhuPay, total, remain, workDays, jhuDays, msg) {
  ...
  $('#outBase')?.textContent = fmt(basePay)
  $('#outJhu')?.textContent = fmt(jhuPay)
  ...
  if (lineEl)
    lineEl.textContent = `실근로일 ${workDays}일 + 유급주휴일 ${jhuDays}일 = 총 ${paidDays}일`

  const elR = $('#outRemain')
  if (elR) {
    elR.textContent = fmt(remain)
